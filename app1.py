...
def app():
    ...
    st.markdown(
        """
        <details>
            <summary>What does optimization method mean?</summary>
    ...
    if run_button:
        ...
        with st.spinner('Calculating optimal locations'):
            ...
            if response1.status_code == 500:
                ...
                avg_distance = response1["avg_distance"]
                coordinate = response1["centers"]
                pop_in_radius = response1.get('Population in Radius')
                percent_in_radius = response1.get('Percent in Radius')

        with st.spinner('Generating Map'):
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric('Number of centers:', len(coordinate))
                with col2:
                    st.metric('Average distance to customers:', f'{round(avg_distance, 1)} Km')
                if pop_in_radius != None:
                    with col3:
                        st.metric(f'Population in radius:', f'{round(pop_in_radius/1000000, 2)} M')
                    with col4:
                        st.metric(f'Percent of population in radius', f'{round(percent_in_radius*100, 1)}%')
            #Get Dataframe
